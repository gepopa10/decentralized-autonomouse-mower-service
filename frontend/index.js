...
const initialize = async () => {
    ...
    nftContractInstance = new web3.eth.Contract(abiNFT, nftContractAddress);
    ...
    async function sendTransaction() {
        var failed = true;
        await robotContractInstance.methods.start_mission().send({
                from: account,
    ...
    async function requestRobotUrl() {
        var failed = true;
        await robotContractInstanceAdapter.methods.requestRobotUrlBytes().send({
                from: account,
                gas: 1000000
            },
            (error, result) => {
    ...
    async function balanceOfNfts() {

        const balance = await nftContractInstance.methods.balanceOf(account).call()
